...
// Use regex to find the messages.length === 0 placeholder block and replace it
// This avoids line-ending issues
const re = /\{messages\.length === 0 && \(\s*<div[^>]*>\s*<div[^>]*>\s*Ready to design\s*<\/div>\s*<div[^>]*>\s*Choose an element type above[\s\S]*?<\/div>\s*<\/div>\s*\)\}/;

if (!re.test(content)) {
  console.error("Regex did not match. Showing text around 'Ready to design':");
  const pos = content.indexOf("Ready to design");
  console.log(JSON.stringify(content.slice(pos - 200, pos + 400)));
  process.exit(1);
}
...
console.log("Done — elem-cards restored successfully.");
console.log("Cards present:", content.includes("RC_ELEMENTS.map(el => ("));
