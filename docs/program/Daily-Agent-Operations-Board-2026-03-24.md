...
|---|---|---|---|---|---|---|---|
| OPS-001 | Agent 01 | Validate repository structure and branch discipline for release candidate | P1 | **BLOCKED** | None | **CRITICAL: No .git directory found — project not yet a git repository. Must run git init, create GitHub remote, and push before CI can execute. Steps added to runbook.** | 2026-03-25 |
...
| OPS-008 | Agent 08 | Publish release documentation pack and operator handoff notes | P1 | In Progress | OPS-007 | Release docs and operator runbook | 2026-03-27 |
| OPS-009 | Agent 09 | Implement staged Azure deployment pipeline with rollback gate | P0 | In Progress | OPS-004, OPS-005, OPS-007 | CI workflow hardened in .github/workflows/ci.yml (backend tests + frontend type/build gates + manual-gated staging/prod deploy jobs + rollback gate). CI YAML bug fixed (secrets context not permitted in if-conditions — now uses env vars). **BLOCKED on: git init + GitHub push (no .git exists). Phase 0 steps added to runbook.** | 2026-03-28 |
