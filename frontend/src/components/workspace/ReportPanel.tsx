...
// Section titles that are "analysis" (results) — excluded from §2, shown in §3
const ANALYSIS_SECTION_TITLES = new Set([
  // RC beam
  "Shear Design", "Deflection Check",
  // RC slab
  "Deflection Check",
  // RC column
  "Slenderness", "Axial Design", "Moment Check",
  // RC foundation
  "Bearing Capacity", "Shear Checks",
  // Steel beam
  "Capacity", "Deflection",
  // Steel column
  "Combined Check",
  ...
  "Capacity Check",
  // Steel portal
  "Analysis Results", "Rafter Check", "Column Check", "Stability & Deflection",
]);

// Input-only sections excluded from both §2 and §3 (handled in §1 or skipped)
const EXCLUDED_INPUT_TITLES = new Set([
  "Project Details", "Bending Design", "Bending (short span)", "Bending (long span)",
]);
...
<body>
  ...
  <div style="border:1.5px solid #f4c4c4;background:#fff5f5;border-radius:5px;padding:10px 14px;margin-top:24px;">
    <div style="font-size:9pt;font-weight:700;color:#b84444;text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px;">Professional Liability Disclaimer</div>
    <p style="font-size:8pt;color:#444;line-height:1.65;margin:0 0 5px;">
      This document has been generated by <strong>SSAD-v1.0</strong> (Smart Structural Analysis &amp; Design), an automated computational tool. Input parameters were extracted from user-provided data with AI-assisted interpretation (${p.moduleLabel}).
    </p>
    <p style="font-size:8pt;color:#444;line-height:1.65;margin:0 0 5px;">
      <u>This output is <strong>not</strong> a certified engineering design.</u> It must be independently reviewed, validated, and approved by a suitably qualified and professionally indemnified <strong>Chartered Structural Engineer</strong> before it may be used as a basis for any construction work, planning submission, building regulations application, or procurement decision.
    </p>
    <p style="font-size:8pt;color:#444;line-height:1.65;margin:0;">
      <strong>Bae Consulting Engineers accepts no responsibility or liability</strong> &mdash; whether in contract, tort, or otherwise &mdash; for any loss, damage, or consequence arising from the use of uncertified outputs produced by this software. The engineer certifying this document assumes full professional responsibility for its contents.
    </p>
  </div>
</body>
</html>`;
      ...
      <div className="scr" style={{ background: "#ddd9d2" }}>
        <div id="rpt-document" style={{ margin: "14px 14px 32px", background: "#fff", border: "1px solid #d4cfc8", borderRadius: 14, overflow: "hidden", boxShadow: "0 6px 30px rgba(0,0,0,.12)" }}>
          ...
          <RptSection num={2} title="Design Input" accent={accent}>
            {(() => {
              // Filter out project details, design output sections, and analysis sections
              const inputSections = sections.filter(s =>
                !EXCLUDED_INPUT_TITLES.has(s.title) &&
                !ANALYSIS_SECTION_TITLES.has(s.title)
              );
          ...
          <RptSection num={3} title="Analysis" accent={accent}>
            {(() => {
              // 1. Collect ANALYSIS_KEYS from any Loading-type section (design forces: M, V etc.)
              const forceEntries: Array<[string, unknown]> = [];
              sections.forEach(sec => {
                if (sec.title.toLowerCase().startsWith("loading")) {
                  Object.entries(sec.content).forEach(([k, v]) => {
                    if (ANALYSIS_KEYS.has(k)) forceEntries.push([k, v]);
                  });
                }
              });

              // 2. All sections whose title is in ANALYSIS_SECTION_TITLES
              const analysisSections = sections.filter(s => ANALYSIS_SECTION_TITLES.has(s.title));

              const hasAny = forceEntries.length > 0 || analysisSections.length > 0;
              if (!hasAny) return (
                ...
                <>
                  {forceEntries.length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontFamily: "var(--mono)", fontSize: 15, fontWeight: 700, color: accent, textTransform: "uppercase", letterSpacing: ".7px", marginBottom: 8, paddingBottom: 4, borderBottom: `1px solid ${accent}22` }}>Design Forces</div>
                      {forceEntries.map(([k, v]) => <KVRow key={String(k)} label={paramLabel(String(k))} value={fmtVal(v)} />)}
                    </div>
                  )}
                  {analysisSections.map(sec => {
                    const entries = Object.entries(sec.content);
                    if (entries.length === 0) return null;
                    return (
                      <div key={sec.title} style={{ marginBottom: 16 }}>
                        <div style={{ fontFamily: "var(--mono)", fontSize: 15, fontWeight: 700, color: accent, textTransform: "uppercase", letterSpacing: ".7px", marginBottom: 8, paddingBottom: 4, borderBottom: `1px solid ${accent}22` }}>{sec.title}</div>
                        {entries.map(([k, v]) => <KVRow key={k} label={paramLabel(k)} value={fmtVal(v)} />)}
                      </div>
                    );
                  })}
                </>
          ...
          <RptSection num={4} title="Design Output" accent={accent}>
            ...
            {(() => {
              ...
              sections.forEach(sec => {
                if (["Bending Design","Bending (short span)","Bending (long span)","Section Design","Steel Section","Connection Design"].includes(sec.title)) {
...
{/* -- � 7  General Interpretation -- */}
          ...
          <div style={{ padding: "16px 18px", background: "#f8f7f5", borderTop: "2px solid #e8e2d9" }}>
            ...
            <div style={{ fontFamily: "var(--ui)", fontSize: 12, color: "var(--dim)", lineHeight: 1.75, borderTop: "2px solid #e0dbd4", paddingTop: 10 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: "#b84444", marginBottom: 4, textTransform: "uppercase", letterSpacing: ".5px" }}>Professional Liability Disclaimer</div>
              <p style={{ margin: "0 0 6px" }}>
                This document has been generated by <strong>SSAD-v1.0</strong> (Smart Structural Analysis &amp; Design), an automated computational tool. Input parameters were extracted from user-provided data with AI-assisted interpretation ({moduleShort}).
              </p>
              <p style={{ margin: "0 0 6px" }}>
                <u>This output is <strong>not</strong> a certified engineering design.</u> It must be independently reviewed, validated, and approved by a suitably qualified and professionally indemnified <strong>Chartered Structural Engineer</strong> before it may be used as a basis for any construction work, planning submission, building regulations application, or procurement decision.
              </p>
              <p style={{ margin: 0 }}>
                <strong>Bae Consulting Engineers accepts no responsibility or liability</strong> — whether in contract, tort, or otherwise — for any loss, damage, or consequence arising from the use of uncertified outputs produced by this software. The engineer certifying this document assumes full professional responsibility for its contents.
              </p>
            </div>
          </div>
