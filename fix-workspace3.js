...
// Workspace home \u2014 AI-first agent cards
  ...
  return (
    <div style={{ flex: 1, display: "flex", overflow: "hidden", background: "linear-gradient(160deg,#0d1b2e 0%,#12294a 100%)" }}>
      ...
      <div className="ws-agents" style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto", padding: "24px 0 16px" }}>
        ...
        {/* RC Agent card \u2014 always accessible */}
        ...
        {/* Steel Agent card \u2014 Pro/Trial only */}
        <div
        ...
        >
          {effectiveTier === "guest" && (
            <span style={{ position: "absolute", top: 10, right: 10, fontFamily: "var(--mono)", fontSize: 8, fontWeight: 700, color: "#c8960c", background: "rgba(200,150,12,.15)", border: "1px solid rgba(200,150,12,.4)", borderRadius: 5, padding: "2px 7px" }}>PRO</span>
          )}
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
...
export default function WorkspaceScreen() {
  ...
  // Clear auth screen once user becomes authenticated (guest, register, login)
  useEffect(() => {
    if (!isLoading && isAuthenticated && screen === "auth") {
      goScreen("workspace");
    }
